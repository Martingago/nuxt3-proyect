<template>
    <form @submit.prevent class="d-flex flex-column container shadow rounded gap-3 p-3">
        ...
        <div class="d-flex flex-column">
            ...
            <textarea id="descripcionProducto" required cols="1" rows="4" v-model="datos_articulo.descripcion_articulo"></textarea>
        </div>
        ...
        <div class="d-flex justify-content-between">
            <div class="d-flex flex-column">
                ...
                <span class="d-flex flex-row justify-content-left align-items-left gap-1">
                    <input type="number" step="0.01"  name="precioProducto" id="precioProducto"
            ...
            <div class="d-flex gap-2">
                <div class="d-flex flex-column">
                    ...
                    <select  @change="hasDiscount($event)" name="decuentoProducto"
                    ...
                    <div class="d-flex flex-column">
                        ...
                        <span class="d-flex gap-1 fw-700">
                            <input type="number" name="porcentajeDescuento" id="precioProducto"
                                v-model="datos_articulo.porcentaje_descuento" :required="datos_articulo.descuento" :disabled="!datos_articulo.descuento">
                            <p class="m-auto fs-5 fw-600">%</p>
                        </span>
                    </div>
                    <div class="d-flex flex-column">
                        <label for="precioAnterior">Precio anterior</label>
                        <span class="d-flex gap-1"><input v-model="datos_articulo.precio_anterior" :required="datos_articulo.descuento" :disabled="!datos_articulo.descuento" type="number" step="0.01" name="precioAnterior"
                                id="precioAnterior">
                            <p class="m-auto fs-5 fw-600">€</p>
                        </span>
                    </div>

                    <button :disabled="!datos_articulo.descuento" class="btn btn-primary p-0 mt-2" @click.prevent="calcularPVP">Calcular
                        PVP</button>
                </div>
        ...
        <div class="notification-area rounded">
            ...
            <p v-if="alertaUpdt" class="alert alert-warning m-0 p-1">Atencion! El precio del producto se ha actualizado de <span class="fw-bold">{{ pvpDesfase }}€</span>  a <span class="fw-bold">{{datos_articulo.precio_venta}}€</span> en base a los descuentos aplicados</p>
        </div>

        <div class="d-flex flex-column">
            <label for="stockProducto">Stock del artículo</label>
            <input type="number" required name="stockProducto" id="stockProducto">
        </div>
        <button @click="subirProducto" class="btn btn-primary">Subir producto</button>
    </form>
</template>
...
const calcularPVP = () => {
    ...
    datos_articulo.value.precio_venta = calculoDescuento;
    }
}
