        ...
        <ol>
            <li v-for="pedido in userStore.user_mostrado" :key="pedido.codigo_pedido" class="mb-3">
                <article class="border border-2 rounded p-2">
                    ...
                    <ul>
                        <li v-for="producto in pedido.productos_pedido" :key="producto.productID">
                            ...
                            <div class="d-flex">
                                ...
                                <p class="mb-0 m-end text-end lh-1">Precio unitario: <span class="fw-600">{{ producto?.price
                                }}€</span></p>
                                <p class="mb-0 text-end lh-1">Cantidad: <span class="fw-600">x{{ producto?.count }}</span>
                                </p>
                            </div>
                            <hr class="my-1">
                    ...
                    <p class="mb-0 d-flex"><span class="flex-grow-1">Total:</span> <span class="fw-600">{{
                        pedido.precio_total }}€</span></p>
                </article>
            </li>
        </ol>
...
onMounted(async () => {
    while (userStore.info === null) {
        await new Promise(resolve => setTimeout(resolve, 50));

    }   
    //Se montan los datos por primera vez al cargar la página 
    ...
    else if (userStore.auth && userStore.user_pedido.length !== userStore.user_mostrado.length) {
        
        start.value = userStore.info.user_orders.length -1;
        userStore.user_mostrado = await getPedidosInfo(userStore.info.user_orders, userStore.user_pedido);
    }
    start.value = userStore.info.user_orders.length - userStore.user_mostrado.length;
})


//Carga nuevos datos de pedidos
